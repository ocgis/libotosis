...
/* These prototypes and definitions are stolen from MiNTlibs and modified */

#ifndef _LINEA_H
#define _LINEA_H

/* Various structures stolen from mintlib's linea.h along with new stuff */
...
/* ------------------------------------------------------------------------- */
  ...
  short   (*_SEEDABORT)(void); /* 118 Pointer to function returning int,     *
                                * which is called by the fill logic to allow *
                                * the fill to be aborted. If the routine     *
                                * returns FALSE (0) the fill is not aborted. *
                                * If it returns TRUE (~0) the fill is        *
                                * aborted.                                   */
/* ------------------------------------------------------------------------- */
...
{
/* Type          Name             Offset         Comments                    */
/* ------------------------------------------------------------------------- */
  short          font_id;        /*  0 Font face identifier  1 == system     *
                                  *    font                                  */
  short          size;           /*  2 Font size in points                   */
  char           name[32];       /*  4 Face name                             */
  short          first_ade;      /* 36 Lowest ADE value in the face (lowest  *
                                  *    ASCII value of displayable character).*/
  short          last_ade;       /* 38 Highest ADE value in the face         *
                                  *    (highest ASCII value of displayable   *
                                  *    character).                           */
  short          top;            /* 40 Distance of top line relative to      *
                                  *    baseline                              */
  short          ascent;         /* 42 Distance of ascent line relative to   *
                                  *    baseline                              */
  short          half;           /* 44 Distance of half line relative to     *
                                  *    baseline       */
  short          descent;        /* 46 Distance of decent line relative to   *
                                  *    baseline                              */
  short          bottom;         /* 48 Distance of bottom line relative to   *
                                  *    baseline.                             *
                                  *    All distances are measured in         *
                                  *    absolute values rather than as        *
                                  *    offsets. They are always +ve          */
  short          max_char_width; /* 50 Width of the widest character in font */
  short          max_cell_width; /* 52 Width of the widest cell character    *
                                  *    cell in face                          */
  short          left_offset;    /* 54 Left Offset see Vdi appendix G        */
  short          right_offset;   /* 56 Right offset   "      "     "         */
  short          thicken;        /* 58 Number of pixels by which to thicken  *
                                  * characters                               */
  short          ul_size;        /* 60 Width in  pixels of the underline     */
  short          lighten;        /* 62 The mask used to lighten characters   */
  short          skew;           /* 64 The mask used to determine when to    *
                                  *    perform additional rotation on the    *
                                  *    character to perform skewing          */
  short          flags;          /* 66 Flags                                 *
                                  *    bit 0 set if default system font      *
                                  *    bit 1 set if horiz offset table       *
                                  *    should be used                        *
                                  *    bit 2 byte-swap flag (thanks to Intel *
                                  *    idiots)                               *
  ...
  short          form_width;     /* 80 Form width (#of bytes /scanline in    *
                                  *    font data)                            */
  short          form_height;    /* 82 Form height (#of scanlines in font    *
                                  *    data)                                 */
